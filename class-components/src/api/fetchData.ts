...
import type {Pokemon, PokemonInfo} from "@/types.ts";

export const fetchPokemons = async <T extends number>(page: T): Promise<PokemonInfo[]> => {
  try {
    ...
    return data.results as PokemonInfo[];
  } catch(error) {
